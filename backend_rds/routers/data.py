...
async def failures_time(request: Request):
    try:
        ...
        with engine.begin() as conn:
            ...
            query = select(class_handler[table_name].c.date, class_handler[table_name]
                           .c.time_to_failure, class_handler[table_name].c.aircraftSerNum_1)
            result = conn.execute(query)
            data = result.fetchall()
...
async def cumlative_time(request: Request):
    try:
        ...
        with engine.begin() as conn:
            # query = text(
            #     "SELECT date,time_to_failure ,aircraftsernum_1 FROM aircraft_data;")
            query = select(class_handler[table_name].c.cumulative_duration).filter(
                class_handler[table_name].aircraftsernum_1 == aircraftsernum_1).order_by(class_handler[table_name].cumulative_duration.desc()).limit(1)
            # query = text(
            #     f"SELECT cumulative_duration FROM aircraft_data WHERE aircraftsernum_1 = {aircraftsernum_1} ORDER BY cumulative_duration DESC LIMIT 1;")
            result = conn.execute(query)
            data = result.fetchall()
